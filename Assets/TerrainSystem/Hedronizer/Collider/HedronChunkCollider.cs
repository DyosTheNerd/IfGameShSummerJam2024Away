...
public class HedronChunkCollider : MonoBehaviour
{
    ...
    public AsyncGPUReadbackRequest BakeAsync(int index){
        listIndexes.Clear();
        listVertices.Clear();
        ...
        return AsyncGPUReadback.Request(hedronizer.computeBuffer, hedronizer.ItensInBuffer * hedronizer.ComputeStrideSize(), 0, 
        (AsyncGPUReadbackRequest request) => {
            Task task = Task.Factory.StartNew(delegate {
                

            var vertices = request.GetData<Vector3>(0);
